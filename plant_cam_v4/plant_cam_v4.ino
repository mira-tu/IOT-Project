...
.btn-row{text-align:center;margin-bottom:14px}
button{
  padding:12px 22px;
  border:none;
  border-radius:999px;
...
#stream{width:100%;display:none;vertical-align:top;aspect-ratio:4/3;object-fit:cover}
...
.ctrl-row{display:grid;grid-template-columns:130px 1fr 42px;align-items:center;gap:12px;margin:10px 0}
.ctrl-row.compact{grid-template-columns:130px auto}
.ctrl-row label{font-size:13px;color:var(--muted);font-weight:700}
.ctrl-row input[type=range]{width:100%;accent-color:var(--leaf)}
.ctrl-row select{width:100%;background:#fff;color:var(--ink);border:1px solid var(--line);padding:10px 12px;border-radius:14px}
.ctrl-row span{text-align:right;font-size:13px;font-weight:700}
.tog{position:relative;display:inline-block;width:54px;height:30px}
.tog input{opacity:0;width:0;height:0}
.knob{position:absolute;cursor:pointer;inset:0;background:rgba(23,54,41,.18);border-radius:999px;transition:.2s}
.knob:before{content:"";position:absolute;width:22px;height:22px;left:4px;bottom:4px;background:#fff;border-radius:50%;transition:.2s;box-shadow:0 5px 12px rgba(0,0,0,.18)}
...
<div class="container">
  ...
  <div class="logic">
    <div class="logic-card">
      <h3>1. Greenery</h3>
      <p>This is the share of sampled pixels where green clearly beats red and blue.</p>
    </div>
    <div class="logic-card">
      <h3>2. Vivid greens</h3>
      <p>This catches stronger leaf-like greens instead of pale grey-green noise.</p>
    </div>
    <div class="logic-card">
      <h3>3. Clear view</h3>
      <p>If the scene is too dark or washed out, the detector marks it as blocked instead.</p>
    </div>
  </div>
  <div class="dex-panel">
  ...
  <div class="controls">
    <h3 style="margin-top:0">Plant Telemetry Controls</h3>
    <div class="ctrl-row">
    ...
    <div class="ctrl-row">
      ...
      <input type="range" id="quality" min="10" max="63" value="12" oninput="camCtrl(this)">
    ...
    <div class="note">Telemetry still runs fully on the ESP32. EcoDex species ID now comes from your offline laptop classifier when the view is clear enough to capture.</div>
  </div>
</div>
...
var testClassifier = async () => {
  ...
  if(ready){
    $('dexMessage').textContent = 'Laptop classifier is online. Aim the camera at one trained plant and EcoDex will identify it.';
  } else {
...
var startStream = function(){
  ...
  poll = poll || setInterval(getStatus, 500);
  classifyTimer = classifyTimer || setInterval(classifyFrame, 1200);
  setDexPending('Telemetry is warming up. EcoDex will classify once the plant view is clear.');
...
var stopStream = function(){
  ...
  clearInterval(classifyTimer); classifyTimer = null;
  resetDexCard();
}
...
var classifyFrame = async (force) => {
  if(!active || classifyBusy) return;
  if(!lastStatus.detected || lastStatus.blocked || lastStatus.age > 2500) return;
  ...
  classifyBusy = true;
  lastClassifyMs = now;
  $('dexMessage').textContent = 'EcoDex is comparing this frame to your offline plant model...';
  ...
  fetch('/status')
    ...
    .then(d => {
      ...
      if(d.blocked){
        ...
        if(!wasDetected && !classifyBusy) setDexPending('Plant-like scene found. EcoDex will try to identify it in a moment.');
        classifyFrame(!wasDetected);
      } else {
        ...
        box.style.display = 'none';
        lastClassifyMs = 0;
        if(!classifyBusy) setDexPending('No plant-like scene yet. Point the camera at one trained plant.');
      }
    })
    .catch(() => {
...
void detectionTask(void *param) {
  for (;;) {
    if (streamActive) {
      runDetection();
      vTaskDelay(pdMS_TO_TICKS(700));   // ~1.4 checks/sec
    } else {
...
void setup() {
  ...
  sensor_t *s = esp_camera_sensor_get();
  s->set_framesize(s, FRAMESIZE_QVGA);   // 320×240 — optimal for detection speed
  s->set_vflip(s,    1);
  s->set_hmirror(s,  1);
  s->set_whitebal(s, 1);
  s->set_awb_gain(s, 1);
